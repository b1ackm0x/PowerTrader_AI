...
class PowerTraderHub(tk.Tk):
    def __init__(self):
        ...
        main_dir = self.settings["main_neural_dir"] or os.path.dirname(self.project_dir)
        self.coin_folders = build_coin_folders(main_dir, self.coins)
    ...
    def _ensure_alt_coin_folders_and_trainer_on_startup(self) -> None:
        ...
        try:
            ...
            main_dir = (self.settings.get("main_neural_dir") or os.path.dirname(self.project_dir) or os.getcwd()).strip()
                    ...
                    if chart:
                        def _do_refresh_visible():
                            try:
                                ...
                                try:
                                    main_dir = self.settings.get("main_neural_dir") or os.path.dirname(self.project_dir)
                                    cf_sig = (main_dir, tuple(self.coins))
    ...
    def _refresh_coin_dependent_ui(self, prev_coins: List[str]) -> None:
        ...
        self.coin_folders = build_coin_folders(self.settings.get("main_neural_dir") or os.path.dirname(self.project_dir), self.coins)
    ...
    def _refresh_neural_overview(self) -> None:
        ...
        try:
            main_dir = self.settings.get("main_neural_dir") or os.path.dirname(self.project_dir)
            sig = (main_dir, tuple(self.coins or []))
